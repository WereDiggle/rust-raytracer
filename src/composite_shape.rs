...
impl Intersectable for SubtractShape {
    ...
    fn get_closest_intersect(&self, ray: Ray) -> Option<Intersect> {
        ...
        if positive_intersects.len() == 0 {
            ...
            return Some(positive_intersects.swap_remove(0));
        }
    ...
    // TODO: refactor this function as it shares a lot with get_closest_intersect
    fn get_all_intersects(&self, ray: Ray) -> Vec<Intersect> {
        let ray = ray.transform(self.transform.get_inverse_transform());
        ...
        // Handle trivial cases
        if positive_intersects.len() == 0 || negative_intersects.len() == 0 {
            return positive_intersects;
        }

        let mut ret_intersects: Vec<Intersect> = Vec::with_capacity(positive_intersects.len() + negative_intersects.len());
        ...
        for (sign, _, intersect) in all_intersects {
            ...
            // positive intersect
            if !in_neg && sign == Sign::Pos {
                ret_intersects.push(intersect.transform(self.transform.get_transform()));
            }
            // subtracted intersect
            else if in_pos && sign == Sign::Neg {
                ret_intersects.push(intersect.transform(self.transform.get_transform())
